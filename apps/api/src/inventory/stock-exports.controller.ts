...
export class StockExportsController {
  ...
  @Get() list(@Query() q: ListExportDto) { return this.service.list(q); }
  @Post() create(@Body() dto: CreateExportDto, @CurrentUser() user: CurrentUser) { return this.service.create(dto, user.id); }
  ...
  @Patch(':id') update(@Param('id') id: string, @Body() dto: CreateExportDto) { return this.service.update(id, dto); }
